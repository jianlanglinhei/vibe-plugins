...
插件同时支持通过 [rulesync](https://github.com/dyoshikawa/rulesync) 分发，可以一键生成 Cursor、Claude Code、Windsurf 等多个 AI IDE 的配置。
...
# 进入插件目录
cd plugins/cross-platform

# 生成 Cursor 配置
rulesync generate --targets cursor --features "*"

# 生成 Claude Code 配置（启用 Modular MCP 减少 token 消耗）
rulesync generate --targets claudecode --features "*" --modular-mcp
```
...
详细说明请参考：[plugins/cross-platform/.rulesync/README.md](plugins/cross-platform/.rulesync/README.md)
